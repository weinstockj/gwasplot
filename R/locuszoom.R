...
query_encode_ccres <- function(
  ...
  if (!is.null(cell_type) && is.null(biosample)) {
    ...
    if (nrow(biosamples_df) > 0) {
      matching_biosample <- biosamples_df[
        grepl(cell_type, biosamples_df$display_name, ignore.case = TRUE),
      ]
      if (nrow(matching_biosample) > 0) {
        ...
        warning(
          "Cell type '",
          cell_type,
          "' not found. Available cell types can be viewed with get_encode_biosamples()"
        )
        return(data.frame())
      }
    }
  ...
  tryCatch(
    {
      ...
      if (httr::status_code(response) != 200) {
        cli::cli_warn(
          "ENCODE SCREEN API request failed with status: ",
      ...
      if (!is.null(result$errors)) {
        cli::cli_abort(
          "ENCODE SCREEN API returned errors: ",
...
#' @export
locuszoom.GWASFormatter <- function(
  x,
  locus_chr,
  locus_start,
  locus_end,
...
locuszoom.data.frame <- function(
  ...
  ccres <- data.frame()
  if (include_ccres) {
    ccres <- query_encode_ccres(
      locus_chr,
      locus_start,
      locus_end,
      assembly = "grch38",
      biosample = ccre_biosample,
      cell_type = ccre_cell_type
    )
  }
  ...
  p_assoc <- ggplot2::ggplot(df, ggplot2::aes(x = POS, y = -log10(PVALUE))) +
    ...
    ggplot2::scale_x_continuous(
      ...
      labels = scales::number(
        seq(min(df$POS), max(df$POS), length.out = 6) / 1e6,
        accuracy = 0.01
      ),
...
search_cell_types <- function(search_term = NULL, assembly = "grch38") {
  ...
  if (!is.null(search_term)) {
    biosamples_df <- biosamples_df[
      grepl(search_term, biosamples_df$display_name, ignore.case = TRUE),
    ]
  }

  # Return relevant columns
  result <- biosamples_df[, c(
    "biosample",
    "display_name",
    "ontology",
    "life_stage",
